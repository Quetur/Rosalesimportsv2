...
router.get("/categoriaborrar/:id", isAuthenticated, async (req, res) => {
    try {
        const { id } = req.params;
        // Fallará si la categoría está en uso por algún producto (Integridad referencial)
        await pool.query("DELETE FROM categoria WHERE id_categoria = ?", [id]);
        res.redirect("/categorialista");
    } catch (error) {
        console.error("Error al eliminar:", error.message);
        res.status(500).send("No se puede eliminar: Verifique si hay productos vinculados.");
    }
});
